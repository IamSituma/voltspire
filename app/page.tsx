...
export default function HomePage() {
  ...
  // FAQ data
  const faqs: FAQ[] = [
    { question: "What is your return policy?", answer: "We accept returns within 14 days of delivery. Products must be in original packaging and unused." },
    { question: "Do you offer international shipping?", answer: "Currently, we only ship within Uganda. International shipping will be available soon." },
    { question: "How long does delivery take?", answer: "Delivery typically takes 2-5 business days depending on your location." },
  ...
  return (
    <main className="flex flex-col gap-16">
      ...
      <section className="relative w-full h-[500px] md:h-[850px]">
        <Image src="/images/banner.png" alt="Hero Banner" fill className="object-cover" />
          ...
          {products.map((product) => (
            <div key={product.id} className="border rounded-lg p-4 flex flex-col bg-gray-50 hover:shadow-md transition">
              <div className="aspect-square overflow-hidden rounded-lg bg-white mb-4">
                <Image src={product.image} alt={product.name} width={300} height={300} className="h-full w-full object-cover" />
              </div>
              <h3 className="font-medium mb-2">{product.name}</h3>
              <p className="text-sm text-gray-700 mb-2">{product.description}</p>
              <div className="flex justify-center gap-2 mb-4">
                <span className="line-through text-muted-foreground">UGX {product.price.toLocaleString()}</span>
                <span className="font-medium text-primary">UGX {product.offerPrice.toLocaleString()}</span>
              </div>
              <div className="flex justify-center gap-2 mt-auto">
                <Button onClick={() => addToCart({ ...product, quantity: 1 })} className="bg-black text-white w-full">
                  Add to Cart
                </Button>
                <Button onClick={() => addToCart({ ...product, quantity: 1 })} className="bg-yellow-400 text-black w-full">
                  Buy Now
                </Button>
              </div>
            </div>
      ...
      <section className="bg-gray-100 py-12">
        ...
        <div className="container mx-auto px-4 grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.map((t) => (
            <div key={t.id} className="bg-white p-6 rounded-lg shadow-md flex flex-col items-center text-center">
              <Image src={t.avatar} alt={t.name} width={80} height={80} className="rounded-full mb-4 object-cover" />
        ...
        <div className="container mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-8">Our Trusted Partners</h2>
            ...
            {partners.map((partner) => (
              <div key={partner.id} className="flex items-center justify-center p-4 bg-white rounded-lg hover:shadow-lg transition duration-300">
